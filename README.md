...
NOTE: You can send an object back as a response to a POST request or follow up the POST with a GET request to retrieve the data. Using a GET request to follow up is more common at Prime and will put you in a better position going into Hard Mode
Finally, build a 'clear' button that resets the whole experience.

HARD MODE:

Keep a historical record of all math operations on the server. Display a list of all previous calculations on the webpage when it loads. Update the list when a new calculation is made.
